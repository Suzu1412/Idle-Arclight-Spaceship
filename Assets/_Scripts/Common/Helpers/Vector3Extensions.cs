...
public static class Vector3Extensions
{
    ...
    public static Vector3 RandomPointInAnnulus2D(this Vector3 origin, float minRadius, float maxRadius)
    {
        float angle = Random.value * Mathf.PI * 2f;
        Vector2 direction = new Vector2(Mathf.Cos(angle), Mathf.Sin(angle));
        ...
        // Converting the 2D direction vector to a 3D position vector
        Vector3 position = new Vector3(direction.x, direction.y, 0f) * distance;
        return origin + position;
    }
}
