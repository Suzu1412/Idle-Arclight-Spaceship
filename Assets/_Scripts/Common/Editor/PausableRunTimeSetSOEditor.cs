...
namespace GameSystemsCookbook
{
    ...
    /// </summary>
    [CustomEditor(typeof(PausableRunTimeSetSO))]

    public class PausableRunTimeSetSOEditor : Editor
    {
        // Reference to the target, cast as a RuntimeSet
        private PausableRunTimeSetSO m_RuntimeSet;
        ...
        private void OnEnable()
        {
            ...
            if (m_RuntimeSet == null)
                m_RuntimeSet = target as PausableRunTimeSetSO;
        ...
        private void BindItem(VisualElement element, int index)
        {
            ...
            label.text = item.GetGameObject() ? item.GetGameObject().name : "<null>";

            // Attach a ClickEvent to the label
            label.RegisterCallback<MouseDownEvent>(evt =>
            {
                // Ping the item in the Hierarchy
                EditorGUIUtility.PingObject(item.GetGameObject());
